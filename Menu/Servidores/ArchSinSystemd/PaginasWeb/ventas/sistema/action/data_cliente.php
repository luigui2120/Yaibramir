	   	...
	   	$sql_registe = mysqli_query($conection,"SELECT COUNT(*) as total_registro FROM cliente WHERE status = 1 ");
			...
			$total_pagina = ceil($total_registro / $por_pagina);
				...
				$query = mysqli_query($conection,"SELECT * FROM cliente
														 WHERE status = 1 ORDER BY idcliente DESC LIMIT $desde,$por_pagina ");
			...
			$total_pagina = ceil($total_registro / $por_pagina);
					...
					$detalleTabla .= '<tr>
						                ...
						                <td class="">
							             ...
							             if ($_SESSION['rol'] == 1 || $_SESSION['rol'] == 2) {
											...
											$detalleTabla.=' | <a class="link_delete" id="eliminarCliente" href="javascript:infoEliminarCliente('.$data['idcliente'].');"><i class="fas fa-trash-alt"></i> Eliminar</a>
								                </td>
								             </tr>';
								         }
				}
				...
				if ($pagina > 1) {
					$lista.= '<li><a href="javascript:listaCliente();"><i class="fas fa-step-backward"></i></a></li>
				<li><a href="javascript:listaCliente('.($pagina-1).');"><i class="fas fa-caret-left"></i></a></li>';
				}
				...
				for ($i=1; $i <= $total_pagina; $i++) { 
						...
						if ($i == $pagina) 
						{
							...
							$lista.= '<li><a href="javascript:listaCliente('.$i.');">'.$i.'</a></li>';
						}
					}

				if ($pagina < $total_pagina) {
					$lista.= '<li><a href="javascript:listaCliente('.($pagina+1).');"><i class="fas fa-caret-right"></i></a></li>
				<li><a href="javascript:listaCliente('.($pagina=$total_pagina).');"><i class="fas fa-step-forward"></i></a></li>';
				}
				$lista.='</ul>';
